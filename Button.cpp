...
Button::Button(color2 fill, point center, unsigned int width, unsigned int height, std::string label) : Quad(fill, center, width, height) {
    this->label = label;
    originalFill = fill;
...
bool Button::isOverlapping(int x, int y) const {
    // TODO: Implement
    if(y >= getTopY() && y <= getBottomY() && getRightX() >= x && getLeftX() <= x) {
        return true;
    }
    else {
        return false;
